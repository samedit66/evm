...
main.add_command(toolchain_group)
...
def new_command(path: Path, library: bool, scoop: bool) -> None:
    ...
    template = LIBRARY_TEMPLATE if library else APPLICATION_TEMPLATE
    project = create_project(ProjectCreationRequest(path, template, scoop=scoop))
...
def init_command(library: bool) -> None:
    ...
    template = LIBRARY_TEMPLATE if library else APPLICATION_TEMPLATE
    project = create_project(ProjectCreationRequest(Path.cwd(), template, initialize=True))
