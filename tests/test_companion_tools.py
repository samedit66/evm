...
def test_installed_gobo_tool_uses_registered_installation(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("evm.companion_tools.list_installations", lambda: (installation,))
    monkeypatch.setattr(
        "evm.companion_tools.current_toolchain_platform",
...
def test_installed_gobo_tool_does_not_trust_path(monkeypatch) -> None:
    monkeypatch.setattr("evm.companion_tools.list_installations", lambda: ())
    monkeypatch.setattr(
        "evm.companion_tools.current_toolchain_platform",
        lambda: SimpleNamespace(identifier="test-platform"),
    )
