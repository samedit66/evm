...
def test_user_store_uses_platform_conventions(
    ...
    monkeypatch.setattr("evm.toolchain_store.os", SimpleNamespace(name="nt"))
    ...
    monkeypatch.setattr("evm.toolchain_store.os", SimpleNamespace(name="posix"))
    monkeypatch.setattr("evm.toolchain_store.platform.system", lambda: "Linux")
    assert user_toolchain_root({"XDG_DATA_HOME": str(tmp_path)}) == (
        tmp_path / "evm" / "toolchains"
    )
    monkeypatch.setattr("evm.toolchain_store.platform.system", lambda: "Darwin")
...
def test_configure_project_toolchains_writes_exact_policy_and_lock(
    ...
    monkeypatch.setattr("evm.toolchain_commands.resolve_artifact", resolve)
...
def test_project_configuration_commands_reject_missing_and_duplicate_policy(
    ...
    monkeypatch.setattr(
        "evm.toolchain_commands.resolve_artifact", lambda selector, client: artifact
    )
