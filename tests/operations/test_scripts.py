...
from evm.dependencies.resolution import resolve_dependencies
...
from evm.operations.scripts import (
    ScriptRunRequest,
    _compile_script,
...
def test_compile_script_materializes_ecf_and_finds_executable(
    ...
    monkeypatch.setattr("evm.operations.scripts._prepare_script_dependencies", lambda *args: None)
    monkeypatch.setattr("evm.operations.scripts.validate_configuration", lambda project: [])
    monkeypatch.setattr("evm.operations.scripts.generate_ecf", lambda project, lock: b"<ecf/>")
    monkeypatch.setattr(
        "evm.operations.scripts.select_toolchain", lambda project, compiler: toolchain
    )
    monkeypatch.setattr("evm.operations.scripts.prepare_build_directory", prepare_build_directory)
    monkeypatch.setattr("evm.operations.scripts.compiler_command", lambda *args: ["gec"])
    monkeypatch.setattr("evm.operations.scripts.run_compiler", lambda *args: None)
    monkeypatch.setattr("evm.operations.scripts.artifact_candidates", lambda *args: (executable,))
...
def test_gobo_rejects_excessive_windows_build_path(
    ...
    monkeypatch.setattr("evm.operations.scripts.sys.platform", "win32")
...
def test_run_script_returns_application_exit_code(
    ...
    monkeypatch.setattr("evm.operations.scripts.prepare_script", lambda request: prepared)
    monkeypatch.setattr(
        "evm.operations.scripts._compile_script",
        lambda selected, request: (toolchain, tmp_path / "app"),
    )
    monkeypatch.setattr(
        "evm.operations.scripts.subprocess.run",
