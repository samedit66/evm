...
from evm.operations.linting import LintRequest, _lint_command, lint_project
...
def test_gelint_command_maps_analysis_options(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "evm.operations.linting.companion_tool", lambda selected, name: Path("/tools/gelint")
    )
...
def test_gelint_uses_registered_gobo_for_ise_semantics(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "evm.operations.linting.installed_gobo_tool", lambda name: Path("/gobo/gelint")
    )
...
def test_lint_project_runs_selected_backend(tmp_path: Path, monkeypatch) -> None:
    ...
    observed = []
    monkeypatch.setattr(
        "evm.operations.linting.select_toolchain", lambda project, compiler: toolchain
    )
    monkeypatch.setattr("evm.operations.linting.prepare_project", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "evm.operations.linting.companion_tool", lambda selected, name: Path("/tools/gelint")
    )
    monkeypatch.setattr(
        "evm.operations.linting.toolchain_environment_values", lambda command: (("GOBO", "/gobo"),)
    )
    monkeypatch.setattr(
        "evm.operations.linting.subprocess.run",
...
def test_lint_project_rejects_incompatible_options(
    ...
    monkeypatch.setattr(
        "evm.operations.linting.select_toolchain", lambda project, compiler: _toolchain("ise")
    )
