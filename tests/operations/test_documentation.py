...
from evm.operations.documentation import (
...
def test_gedoc_command_generates_html_in_requested_directory(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "evm.operations.documentation.companion_tool", lambda selected, name: Path("/tools/gedoc")
    )
...
def test_gedoc_uses_gobo_tool_with_ise_semantics(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(
        "evm.operations.documentation.installed_gobo_tool", lambda name: Path("/gobo/gedoc")
    )
...
def test_document_project_creates_default_output_and_preserves_output(
    ...
    monkeypatch.setattr(
        "evm.operations.documentation.select_toolchain", lambda project, compiler: toolchain
    )
    monkeypatch.setattr(
        "evm.operations.documentation.prepare_project", lambda *args, **kwargs: None
    )
    monkeypatch.setattr(
        "evm.operations.documentation.toolchain_environment_values", lambda command: ()
    )
    monkeypatch.setattr(
        "evm.operations.documentation.subprocess.run",
...
def test_document_project_rejects_unknown_backend(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(
        "evm.operations.documentation.select_toolchain", lambda project, compiler: _toolchain("ise")
    )
