...
def test_detection_reports_missing_failed_and_unversioned_compilers(
    ...
    monkeypatch.setattr("evm.toolchain.selection.shutil.which", lambda name: None)
    assert detect("gobo").error == "gec was not found in PATH"

    monkeypatch.setattr("evm.toolchain.selection.shutil.which", lambda name: f"/tools/{name}")
    monkeypatch.setattr(
        "evm.toolchain.selection.subprocess.run",
...
def test_selection_reports_all_rejection_reasons(
    ...
    monkeypatch.setattr(
        "evm.toolchain.selection.detect_all",
        lambda: {
...
def test_run_compiler_sets_runtime_environment_and_reports_output(
    ...
    monkeypatch.setattr("evm.toolchain.selection.subprocess.run", run)
    monkeypatch.setattr("evm.toolchain.selection.toolchain_environment_values", lambda command: ())
